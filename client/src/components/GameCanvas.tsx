    ...
    <group ref={pcRef} position={position}>
      ...
      {shape === 'rack' ? (
        [0.5, 0.15, -0.15, -0.5].map((yOffset, i) => (
          <mesh key={i} position={[0, height / 2 + yOffset, depth / 2 + 0.01]}>
      ...
      {isActive && (
        <>
          <FloatingCoin position={[0, height, 0]} />
          <MiningParticles position={[0, height / 2, 0]} count={4} />
          <pointLight 
            position={[0, height / 2, 0]} 
            color={color}
      ...
      {/* Power/Status Light */}
      <mesh position={[width / 2 - 0.1, height - 0.2, depth / 2 + 0.02]}>
