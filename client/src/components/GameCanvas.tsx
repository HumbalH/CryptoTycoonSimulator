...
function MiningPC({ position, type, token, isActive, id, onPCClick }: MiningPCProps) {
  ...
  return (
    <group ref={pcRef} position={position}>
      ...
      {/* Main PC Tower */}
      <mesh castShadow position={[0, height / 2 - 2.4, 0]}>
        {shape === 'flat' ? (
      ...
      {/* Front Panel Glow */}
      <mesh position={[0, height / 2 - 2.4, depth / 2 + 0.02]}>
      ...
      {(type === 'gaming' || type === 'quantum') && isActive && (
        <>
          <mesh position={[-width / 2 - 0.02, height / 2 - 2.4, 0]}>
      ...
      {type === 'quantum' && isActive && (
        <mesh position={[0, height / 2 - 2.4, 0]}>
      ...
      {shape === 'rack' ? (
        [0.5, 0.15, -0.15, -0.5].map((yOffset, i) => (
          <mesh key={i} position={[0, height / 2 - 2.4 + yOffset, depth / 2 + 0.01]}>
      ...
      {isActive && (
        <>
          <FloatingCoin position={[0, height - 2.4, 0]} />
          <MiningParticles position={[0, height / 2 - 2.4, 0]} count={4} />
          <pointLight 
            position={[0, height / 2 - 2.4, 0]} 
            color={color}
      ...
      {/* Power/Status Light */}
      <mesh position={[width / 2 - 0.1, height - 2.4 - 0.2, depth / 2 + 0.02]}>
...
function Worker({ id, pcs }: WorkerProps) {
  ...
  useFrame((state, delta) => {
    ...
    if (distance > 0.5) {
      ...
      workerRef.current.position.y = -2.2 + Math.abs(Math.sin(state.clock.elapsedTime * 5)) * 0.1;
    } else {
...
function Floor({ size }: { size: number }) {
  return (
    <>
      ...
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, -2.51, 0]}>
